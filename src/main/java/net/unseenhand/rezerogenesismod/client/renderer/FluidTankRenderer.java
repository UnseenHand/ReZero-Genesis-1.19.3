package net.unseenhand.rezerogenesismod.client.renderer;
...
public class FluidTankRenderer {
    ...
    private static void drawTiledSprite(PoseStack poseStack,
    ...
    private static void drawTextureWithMasking(Matrix4f matrix,
        ...
        bufferBuilder.vertex(matrix, x, y + 16, z)
                .uv(uMin, vMax)
                .endVertex();
        bufferBuilder.vertex(matrix, x + 16 - maskRight, y + 16, z)
                .uv(uMax, vMax)
                .endVertex();
        bufferBuilder.vertex(matrix, x + 16 - maskRight, y + maskTop, z)
                .uv(uMax, vMin)
                .endVertex();
        bufferBuilder.vertex(matrix, x, y + maskTop, z)
                .uv(uMin, vMin)
                .endVertex();
        tessellator.end();
    }
    ...
    public List<Component> getTooltip(FluidStack fluidStack, TooltipFlag tooltipFlag) {
        ...
        try {
            ...
            String mB = String.format("%s", milliBuckets);
            String c = String.format("%s", capacity);

            // nf.format(milliBuckets) and nf.format(capacity) displays 'space' with the wrong symbol
            // That is why for now I use string format

            if (tooltipMode == TooltipMode.SHOW_AMOUNT_AND_CAPACITY) {
                MutableComponent amountString =
                        Component.translatable(
                                "rezerogenesismod.tooltip.liquid.amount.with.capacity",
                                mB, c);
                tooltip.add(amountString.withStyle(ChatFormatting.GRAY));
            } else if (tooltipMode == TooltipMode.SHOW_AMOUNT) {
                MutableComponent amountString =
                        Component.translatable(
                                "rezerogenesismod.tooltip.liquid.amount",
